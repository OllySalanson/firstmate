...
other-session:fm-design" \
  ...
  assert_grep "send-keys -t $(fm_test_fake_tmux_pane firstmate:fm-design) -l : Firstmate instruction waiting:" "$LOG" "send did not ring the window recorded in this home's meta"
  ...
  assert_no_grep "send-keys -t $(fm_test_fake_tmux_pane other-session:fm-design)" "$LOG" "send targeted a foreign same-named window"
