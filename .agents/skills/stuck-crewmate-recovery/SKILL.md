...
Ship and scout relaunches pass the memory gate like fresh spawns, so after a machine freeze relaunch the recorded tasks one at a time: a relaunch the gate defers exits 75 with its task untouched, and the memory watchdog's room notice names it as `<id>(relaunch)` when it fits ([`docs/configuration.md`](../../../docs/configuration.md) "Memory gate").
