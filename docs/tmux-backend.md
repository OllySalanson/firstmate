...
A standard `session:window.pane` target still resolves to that exact pane, but a window whose full name contains the dot wins when it exists, and an absent window or pane index reads as absent.
